...
glGame.baseclass.extend({
    ...
    reqEnterArea() {
        switch (this.node.name) {
            ...
            case 'esydselect':
                ...
                break;
            case 'fishselect':
                this.gameID = glGame.scenetag.FISH;
                glGame.readyroom.reqEnterArea(glGame.scenetag.FISH);
                break;
            case 'jszjhselect':
    ...
    enterGame(level) {
        glGame.room.reqMyGameState(this.gameID, level).then(() => {
            ...
            if (gameID == glGame.scenetag.FISH) {
                if (glGame.user.get("coin") >= this["gameConfig"][level].EntranceRestrictions) {
                    glGame.room.sendEnterRoom(level);
                } else {
                    let string = ` <color=#99C7FF>       您的金币不足，该房间需要</c> <color=#ff0000> ${this.getFloat(this.gameConfig[level].EntranceRestrictions)}  </c><color=#99C7FF>金币才可下注，是否马上前往商城兑换金币？</c>`
