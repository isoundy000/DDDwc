...
glGame.baseclass.extend({
    ...
    onLoad() {
        ...
        if (glGame.user.get("headList")[this.sex]) {
            ...
            glGame.user.reqGetHeadList(this.sex, 8);
        }
    },
    // 初始化界面数据
    resetData() {
    ...
    showUI(index) {
        ...
        let count = keys.length > 8 ? 8 : keys.length;
    ...
    click_sex(index) {
        ...
        if (glGame.user.get("headList")[index]) {
            ...
            glGame.user.reqGetHeadList(index, 8);
        }
    },
    click_close() {
        this.remove();
    ...
    click_confirm() {
        ...
        glGame.user.reqEditHead(this.clickHeadItemID, () => {
            if (this.sex != glGame.user.get("userSex")) {
                glGame.emitter.emit("changeSexIcon", this.sex);
                cc.log("更改了性别", this.sex)
            }
            this.remove();
        });
    },
