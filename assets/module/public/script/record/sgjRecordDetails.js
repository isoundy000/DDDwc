...
glGame.baseclass.extend({
    ...
    updategameinfo(){
        console.log("这是水果机的中奖金额",this.recordDetailsData)
    ...
    updateChipinfo(){
        let chipinfo = JSON.parse(this.recordDetailsData.record).chipObj
        for(let i=1;i<=8;i++){
    ...
    updateRewar(){
        let rewardArr = JSON.parse(this.recordDetailsData.record).rewardArr;
        for(let i=0;i<rewardArr.length;i++){
            let resultItem = cc.instantiate(this.resultItem);
            resultItem.getChildByName("icon").getComponent(cc.Sprite).spriteFrame = this.fruitAtlas._spriteFrames["icon_"+rewardArr[i].fruitType];
            resultItem.getChildByName("number").getComponent(cc.Label).string = "X"+ rewardArr[i].multiple;
