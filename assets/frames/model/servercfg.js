...
server.repetitionSend = function () {
    ...
    if (this.servercount >= 2) {
        let localTag = this.localTag
        this.localTag.cfgurl = /^(https)/.test(localTag.cfgurl) ? localTag.cfgurl.replace(/https/, "http") : localTag.cfgurl.replace(/http/, "https");
        this.servercount = 0;
    }
    if (this.localTag) this.loadSettingCb(this.localTag);
    ...
    xhr.onreadystatechange = () => {
        if (xhr.readyState === 4 && (xhr.status >= 200 && xhr.status < 300)) {
            ...
            if (head && head.code == 0) {
                ...
                glGame.emitter.emit("updateServerCfg");
            }
...
server.loadSettingCB = function (data) {
    ...
    xhr.onreadystatechange = () => {
        ...
        if (xhr.readyState === 4 && (xhr.status >= 200 && xhr.status < 300)) {
            ...
            // this.servercfg.platSvrPort = 8127;
            // this.servercfg.platSvrHost = '192.168.30.111';
